class Widget < ActiveRecord::Base
  ...
  validates :user_id, presence: true
	...
	def get_gmail
		...
		if self.updated_at + self.properties["gmail_expires_in"] - 5 < Time.now || self.properties["gmail_access_token"].blank?
	...
	private 
		def refresh_gmail_access_token
			...
			request.set_form_data({refresh_token: self.properties["gmail_refresh_token"], 
			...
			self.properties["gmail_access_token"] = response["access_token"]
